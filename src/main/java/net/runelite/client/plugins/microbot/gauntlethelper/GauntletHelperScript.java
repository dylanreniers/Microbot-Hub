...
public class GauntletHelperScript extends Script {
    ...
    private static final Set<Integer> HUNLLEF_IDS = Set.of(9035, 9036, 9037, 9038, // Corrupted Hunllef variants
            9021, 9022, 9023, 9024  // Crystalline Hunllef variants
    );
    private static final Set<Integer> DANGEROUS_TILES = Set.of(36047, 36048, // Corrupted tiles (Ground object)
            36150, 36151 // Gauntlet tiles (Ground object)
    );
    private static final Set<Integer> FLOOR_TILES = Set.of(36149, 36046 // CG, G Floor Tiles
    );
    private static final int[] BOW_IDS = {ItemID.GAUNTLET_RANGED_T3_HM, ItemID.GAUNTLET_RANGED_T3, ItemID.GAUNTLET_RANGED_T2_HM, ItemID.GAUNTLET_RANGED_T2, ItemID.GAUNTLET_RANGED_T1_HM, ItemID.GAUNTLET_RANGED_T1};
    private static final int[] STAFF_IDS = {ItemID.GAUNTLET_MAGIC_T3_HM, ItemID.GAUNTLET_MAGIC_T3, ItemID.GAUNTLET_MAGIC_T2_HM, ItemID.GAUNTLET_MAGIC_T2, ItemID.GAUNTLET_MAGIC_T1_HM, ItemID.GAUNTLET_MAGIC_T1};
    private static final int[] HALBERD_IDS = {ItemID.GAUNTLET_MELEE_T3_HM, ItemID.GAUNTLET_MELEE_T3, ItemID.GAUNTLET_MELEE_T2_HM, ItemID.GAUNTLET_MELEE_T2, ItemID.GAUNTLET_MELEE_T1_HM, ItemID.GAUNTLET_MELEE_T1};
    ...
    private final long TIME_PRAYOFFENCE = -1;
            ...
            try {
                ...
                switch (gh_state) {
                    ...
                    case fighting:
                        ...
                        if (now - MAGIC_BLAST_END < BUFFER_MAGICBLAST)
                        ...
                        if ((now - TIME_PRAYPROTECT < CD_RECENTLY) || (now - TIME_PRAYOFFENCE < CD_RECENTLY) || (now - TIME_STEEL < CD_RECENTLY))
                            break; //Prayers changed, return simulates small sleep

                        //Slow Section
                        if (now - TIME_EAT_ATTEMPTED > CD_EAT) checkFood();
                        if (now - TIME_EAT_ATTEMPTED < CD_RECENTLY) {
                            checkPrayerPotions();
                            break;
    ...
    public void checkNPC() {
        ...
        hunllef = Rs2Npc.getNpcs().filter(npc -> HUNLLEF_IDS.contains(npc.getId())).findFirst().orElse(null);
    ...
    public void anim_magic_seen() {
        if (nextPrayer == null) nextPrayer = Rs2PrayerEnum.PROTECT_MAGIC;
    }

    public void anim_range_seen() {
        if (nextPrayer == null) nextPrayer = Rs2PrayerEnum.PROTECT_RANGE;
    }

    public void proj_mageblast_end() {
        MAGIC_BLAST_END = System.currentTimeMillis();
    }

    public void proj_magic_seen() {
        nextPrayer = Rs2PrayerEnum.PROTECT_MAGIC;
    }

    public void proj_range_seen() {
    ...
    private void checkPrayerPotions() {
        ...
        if (currentPrayer < config.ppotvalue()) {
