package net.runelite.client.plugins.microbot.custom.woodcutting;
...
import net.runelite.client.plugins.microbot.custom.woodcutting.enums.ForestryEvents;
import net.runelite.client.plugins.microbot.custom.woodcutting.enums.WoodcuttingPrimaryAction;
import net.runelite.client.plugins.microbot.custom.woodcutting.enums.WoodcuttingScriptState;
import net.runelite.client.plugins.microbot.custom.woodcutting.enums.WoodcuttingSecondaryAction;
import net.runelite.client.plugins.microbot.custom.woodcutting.enums.WoodcuttingTree;
import net.runelite.client.plugins.microbot.custom.woodcutting.enums.WoodcuttingTreeLocations;
import net.runelite.client.plugins.microbot.custom.woodcutting.enums.WoodcuttingWalkBack;
...
import static net.runelite.api.gameval.AnimationID.FORESTRY_CAMPFIRE_BURNING_LOGS;
import static net.runelite.api.gameval.AnimationID.FORESTRY_CAMPFIRE_BURNING_MAGIC_LOGS;
import static net.runelite.api.gameval.AnimationID.FORESTRY_CAMPFIRE_BURNING_MAHOGANY_LOGS;
import static net.runelite.api.gameval.AnimationID.FORESTRY_CAMPFIRE_BURNING_MAPLE_LOGS;
import static net.runelite.api.gameval.AnimationID.FORESTRY_CAMPFIRE_BURNING_OAK_LOGS;
import static net.runelite.api.gameval.AnimationID.FORESTRY_CAMPFIRE_BURNING_REDWOOD_LOGS;
import static net.runelite.api.gameval.AnimationID.FORESTRY_CAMPFIRE_BURNING_TEAK_LOGS;
import static net.runelite.api.gameval.AnimationID.FORESTRY_CAMPFIRE_BURNING_WILLOW_LOGS;
import static net.runelite.api.gameval.AnimationID.FORESTRY_CAMPFIRE_BURNING_YEW_LOGS;
import static net.runelite.api.gameval.AnimationID.HUMAN_CREATEFIRE;
...
public class AutoWoodcuttingScript extends Script {
    ...
    public boolean run(AutoWoodcuttingConfig config) {
        ...
        mainScheduledFuture = scheduledExecutorService.scheduleWithFixedDelay(() -> {
            try {
                if (preFlightChecksAreInvalid(config)) {
                    return;
                }

                switch (woodcuttingScriptState) {
                    case WOODCUTTING:
                        log.info("Woodcutting");
                        if (treeCannotBeCut(config)) {
                            log.info("Cannot cut another tree.");
                            return;
                        }
                        log.info("Cutting another tree");
    ...
    private void handleWoodcutting(AutoWoodcuttingConfig config) {
        ...
        if (tree != null) {
            log.info("Found tree at: {}", tree.getWorldLocation());
    ...
    private boolean treeCannotBeCut(AutoWoodcuttingConfig config) {
        ...
        if (Rs2Inventory.isFull()) {
            if (!Rs2LogBasket.isBasketFull() && Rs2LogBasket.fillLogBasket()) {
                Rs2Antiban.actionCooldown();
                return false;
            }
            woodcuttingScriptState = WoodcuttingScriptState.RESETTING;
    ...
    private void burnLog(AutoWoodcuttingConfig config) {
        ...
        } else if (!isFiremaking() && useCampfire) {
        ...
        sleepUntil(() -> !isFiremaking());
        if (!isFiremaking()) {
            sleepUntil(() -> cannotLightFire, 1500);
        }
        if (!cannotLightFire && isFiremaking()) {
