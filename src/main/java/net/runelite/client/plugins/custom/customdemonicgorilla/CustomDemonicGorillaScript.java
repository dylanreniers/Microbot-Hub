...
// Custom build - optimized for fewer redundant allocations and more human-like pacing
public class CustomDemonicGorillaScript extends Script {
    ...
    // Inventory setups are expensive to build; cache them once instead of re-creating every loop tick
    ...
    private Rs2InventorySetup bankingGear = null;
    ...
    /**
     * Validates config each tick (cheap) but only performs the heavy one-time setup
     * (building inventory setups, disabling the QoL plugin) a single time per run.
     *
     * @return true when the script is safe to continue, false when it has been shut down.
     */
    private boolean preflightChecks(CustomDemonicGorillaConfig config) {
        ...
        if (!initialized) {
            ...
            bankingGear = new Rs2InventorySetup(config.gearSetup(), mainScheduledFuture);

            Plugin qolPlugin = Microbot.getActiveMicrobotPlugins().stream()
                    .filter(p -> p.getClass().getSimpleName().contains("QoLPlugin"))
                    .findFirst().orElse(null);
            if (qolPlugin != null) {
                Microbot.stopPlugin(qolPlugin.getClass());
            }
            initialized = true;
        }
        return true;
    ...
    private void handleTravel(CustomDemonicGorillaConfig config) {
    ...
    private void handleBanking(CustomDemonicGorillaConfig config) {
        Rs2InventorySetup inventorySetup = bankingGear;
        switch (bankingStep) {
            case BANK:
            ...
            case LOAD_INVENTORY:
                ...
                if (equipmentLoaded && inventoryLoaded) {
                    ...
                    while (Rs2Player.getHealthPercentage() < 70 || ateFood || drankPrayerPot) {
                        ...
                        ate = true;
                        sleep(Rs2Random.between(900, 1400));
    ...
    private void handleFighting(CustomDemonicGorillaConfig config) {
    ...
    private void retreatToSafety(CustomDemonicGorillaConfig config) {
        currentTarget = null;
    ...
    private void handleNewTarget(CustomDemonicGorillaConfig config) {
        if (!lootAttempted) {
            Rs2Player.eatAt(80);
    ...
    private void attackGorilla(CustomDemonicGorillaConfig config) {
        if (currentTarget == null || currentTarget.getNpc().isDead()) {
    ...
    private void handleDemonicGorillaAttacks(CustomDemonicGorillaConfig config) {
        ...
        if (currentTarget != null && !currentTarget.getNpc().isDead()) {
            ...
            if (currentAnimation == DEMONIC_GORILLA_AOE_ATTACK && demonicGorillaRockPosition != null) {
                ...
                dangerousWorldPoints.addAll(CustomDemonicGorillaPlugin.lastLocation.getAll());
                ...
                if (safeTile != null) {
                    ...
                    sleep(Rs2Random.between(1400, 1700));
                }
            ...
            if (!dodgedRock) {
                if ((currentGear == ArmorEquiped.RANGED || currentGear == ArmorEquiped.MAGIC) && (currentAnimation != DEMONIC_GORILLA_MELEE_ATTACK && currentAnimation != -1 && currentDefensivePrayer != Rs2PrayerEnum.PROTECT_MELEE)) {
                    ...
                    boolean isMeleeDist = Microbot.getClientThread().runOnClientThreadOptional(() ->
    ...
    private void handleGearSwitching(CustomDemonicGorillaConfig config) {
        try {
            if (failedCount >= 3) {
                currentTarget = getTarget(true);
    ...
    private void activateOffensivePrayer(CustomDemonicGorillaConfig config) {
    ...
    private void switchGear(CustomDemonicGorillaConfig config, HeadIcon combatNpcHeadIcon) {
        ...
        switch (combatNpcHeadIcon) {
            case RANGED:
                if (useMelee && useMagic) {
                    currentGear = Rs2Random.dicePercentage(50) ? ArmorEquiped.MELEE : ArmorEquiped.MAGIC;
                } else if (useMelee) {
            ...
            case MAGIC:
                if (useRange && useMelee) {
                    currentGear = Rs2Random.dicePercentage(50) ? ArmorEquiped.RANGED : ArmorEquiped.MELEE;
                } else if (useRange) {
            ...
            case MELEE:
                if (useRange && useMagic) {
                    currentGear = Rs2Random.dicePercentage(50) ? ArmorEquiped.RANGED : ArmorEquiped.MAGIC;
                } else if (useRange) {
                    currentGear = ArmorEquiped.RANGED;
                } else if (useMagic) {
                    currentGear = ArmorEquiped.MAGIC;
    ...
    private boolean shouldRetreat(CustomDemonicGorillaConfig config) {
        ...
        // We're out of prayer potions when no inventory item matches a known prayer-potion variant
    ...
    private void evaluateAndConsumePotions(CustomDemonicGorillaConfig config) {
