package net.runelite.client.plugins.custom.chinhunter.scripts;
...
import net.runelite.client.plugins.custom.chinhunter.ChinHunterConfig;
...
@Slf4j
public class ChinHunterScript extends AbstractScript {

    private static final int MAX_WALK_ATTEMPTS = 5;
    private static final int WALK_ARRIVAL_TIMEOUT_MS = 3000;
    ...
    private void layTraps() {
        allBoxesOriginalPoints.forEach(tile -> {
            ...
            // Only lay once we've confirmed the player is standing exactly on the intended tile.
            // Otherwise a client freeze/lag would cause the trap to be laid at the player's current
            // (wrong) location instead of the original box position.
            if (!walkToTileAndConfirmArrival(boxTrapLocation)) {
                log.warn("Could not reach box trap tile {} - skipping lay to keep original positions.", boxTrapLocation);
                return;
            }

            if (!boxTiles.contains(boxTrapLocation)) {
    ...
    /**
     * Walks to {@code target} and only returns {@code true} once the player is standing exactly on
     * that tile. Retries the walk a few times to ride out client freezes/lag. This is the guard that
     * guarantees traps are always laid on their original positions - never wherever the player
     * happens to be when a "Lay" is issued.
     */
    private boolean walkToTileAndConfirmArrival(WorldPoint target) {
        for (int attempt = 0; attempt < MAX_WALK_ATTEMPTS; attempt++) {
            ...
            if (sleepUntil(() -> Rs2Player.getWorldLocation().equals(target), WALK_ARRIVAL_TIMEOUT_MS)) {
                return true;
            }
            log.info("Not on target tile {} yet (attempt {}), retrying walk.", target, attempt + 1);
        }
    ...
    private void handleTickManipulationState() {
        ...
        if (!walkToTileAndConfirmArrival(firstBoxTrap.getWorldLocation())) {
            log.warn("Could not reach first trap tile {} - aborting tick manipulation to keep original positions.", firstBoxTrap.getWorldLocation());
            return;
        }
    ...
    private void layTrap(Rs2TileObjectModel box, boolean isFirst) {
        ...
        if (!walkToTileAndConfirmArrival(box.getWorldLocation())) {
            log.warn("Could not reach trap tile {} - skipping lay to keep original positions.", box.getWorldLocation());
            return;
        }
        if (!isFirst) {
