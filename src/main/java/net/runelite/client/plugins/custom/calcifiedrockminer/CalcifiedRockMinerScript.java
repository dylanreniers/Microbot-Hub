package net.runelite.client.plugins.custom.calcifiedrockminer;
...
public class CalcifiedRockMinerScript extends Script {
    ...
    public boolean run(CalcifiedRockMinerConfig config) {
        ...
        this.config = config;
        log.info("Focus on weeping? {}", config.focusCrackedWaterDeposits());
    ...
    private void handleMining() {
        ...
        if (hopIfTooManyPlayersNearby()) return; // Exit current cycle after hop
        ...
        if (config.focusCrackedWaterDeposits() && !Rs2Player.isMoving()) {
            ...
            log.info("Found weeping rocks? {}", weepingRocks != null && !weepingRocks.isEmpty());
    ...
    private boolean hopIfTooManyPlayersNearby() {
    ...
    private void handleBanking() {
        if (!config.dropDeposits()) {
            ...
            Rs2Bank.depositAllExcept("Blessed bone shards");
            Rs2Bank.closeBank();
