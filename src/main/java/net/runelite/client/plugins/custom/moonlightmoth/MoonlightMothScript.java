...
public class MoonlightMothScript extends AbstractScript {
    ...
    private void checkState(MoonlightMothConfig config) {
        ...
        if (Rs2Inventory.isFull() && !hasButterflyJars) {
            logOnceToChat("Inventory is full. Proceeding to BANKING.", false);
    ...
    private void handleCatching(MoonlightMothConfig config) {
        if (!Rs2Player.isAnimating() && !Rs2Player.isInteracting()) {
            ...
            Rs2NpcModel mothToCatch = rs2NpcCache
                    .query()
                    .withId(NpcID.MOTH_MOONLIGHT)
                    .where(moth -> !excludedArea.contains(moth.getWorldLocation()))
                    .nearestOnClientThread();

            if (Objects.nonNull(mothToCatch)) {
                var beforeCount = Rs2Inventory.count(ItemID.BUTTERFLY_JAR_MOONMOTH);
                Microbot.getClientThread().invoke(() -> mothToCatch.click("Catch"));
                sleepUntil(() -> Rs2Inventory.count(ItemID.BUTTERFLY_JAR_MOONMOTH) == beforeCount + 1, 30000);
                totalCaught++;
                sleep(600, 2000);
            }
        }
