...
import net.runelite.client.plugins.custom.motherloadmine.enums.MLMMiningSpot;
import net.runelite.client.plugins.custom.motherloadmine.enums.MLMStatus;
import net.runelite.client.plugins.custom.motherloadmine.enums.Pickaxe;
...
public class MotherloadMineScript extends Script
{
	...
	private final DonderMotherloadMinePlugin plugin;
	...
	@Inject
	public MotherloadMineScript(DonderMotherloadMinePlugin plugin, MotherloadMineConfig config, Rs2TileObjectCache rs2TileObjectCache, Rs2PlayerCache rs2PlayerCache)
	{
		this.plugin = plugin;
		this.config = config;
    ...
    /**
     * Layers extra human-like behaviour on top of the mining antiban template.
     * <p>
     * The default {@code applyMiningSetup()} leaves {@code takeMicroBreaks} off (which makes
     * {@link Rs2Antiban#takeMicroBreakByChance()} a no-op) and does not wander the mouse. When the
     * user opts in we enable short micro-breaks plus idle mouse movement so activity is not a
     * perfectly steady stream of clicks. Real logout breaks are still delegated to the Break Handler
     * plugin, which pauses this script via {@code Microbot.pauseAllScripts}.
     */
    private void applyHumanizationSettings()
    {
        if (!config.useMicroBreaks())
        {
            return;
        }
        ...
        Rs2AntibanSettings.microBreakChance = Math.max(0, Math.min(100, config.microBreakChance())) / 100.0;

        // Keep the duration bounds sane (low <= high, both >= 1 minute) regardless of config input.
        int low = Math.max(1, config.microBreakDurationLow());
        int high = Math.max(low, config.microBreakDurationHigh());
        Rs2AntibanSettings.microBreakDurationLow = low;
        Rs2AntibanSettings.microBreakDurationHigh = high;
    }
    ...
    private void handlePickaxeSpec() {
        if (!config.useSpecialAttack() || !Rs2Equipment.isWearing(SPEC_PICKAXES)) {
            return;
        }
        // Rolling per vein (instead of firing on cooldown every time) avoids the very robotic
        // pattern of specialing the instant 100% energy is available, every single rock, forever.
        if (!Rs2Random.dicePercentage(Math.max(0, Math.min(100, config.specChance())))) {
            return;
        }
	...
	private void handleMining()
	{
		...
		// Between veins is a natural point for a human to pause, so roll for a micro-break here
		// rather than only after emptying the sack (which clustered all breaks at one rare event).
		if (config.useMicroBreaks()) {
	...
	{
		// pauseAllScripts already covers Break Handler breaks, but guard on isBreakActive() too so we
		// stop issuing actions the instant a break begins even if the pause flag lags a tick.
		if (!Microbot.isLoggedIn() || Microbot.pauseAllScripts.get() || BreakHandlerScript.isBreakActive()
    ...
    private void depositHopper()
    {
        ...
        if (hopper != null && hopper.click()) {
			...
			// Jitter the empty-sack trigger instead of always firing at exactly one inventory's worth
			// of headroom, so the decision point is not perfectly predictable run to run.
			shouldEmptySack = effectiveSackAmount >= (maxSackSize - Rs2Random.between(20, 40));
    ...
    private void selectMiningSpotFromConfig() {
        ...
        if (selected == MLMMiningSpot.ANY) {
            if (config.mineUpstairs()) {
                miningSpot = Rs2Random.betweenInclusive(0, 1) == 0 ? MLMMiningSpot.WEST_UPPER : MLMMiningSpot.EAST_UPPER;
            }
            else {
    ...
    {
        // Humans don't always walk to the single closest rock; occasionally pick one of the
        // nearest few instead. Falls back to strict nearest-reachable so behaviour is unchanged
        // when the option is off or no alternative is available.
        if (config.randomizeVeinSelection() && Rs2Random.dicePercentage(30))
        {
            Rs2TileObjectModel nearbyVein = pickRandomNearbyVein();
            if (nearbyVein != null)
            {
                return nearbyVein;
            }
    ...
    private Rs2TileObjectModel pickRandomNearbyVein()
    {
        ...
        List<Rs2TileObjectModel> veins = rs2TileObjectCache.query().where(this::isValidVein).toList();
        if (veins == null || veins.size() < 2)
        {
            return null;
        }

        veins.sort(Comparator.comparingInt(v -> v.getWorldLocation().distanceTo(playerLoc)));
        int poolSize = Math.min(3, veins.size());
        return veins.get(Rs2Random.betweenInclusive(0, poolSize - 1));
    }
